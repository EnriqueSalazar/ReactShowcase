/**
 * Created by enriq on 2/3/2017.
 */
import * as MeisterActions from "./index.js";
const data = [
    {
        id: 1,
        type: "car",
...
test("Filter test for Color", () => {
    const type = 'car';
    const brand = 'Ferrari F40';
    expect(MeisterActions.filter(data, {type, brand}, 'colors')[0]).toEqual(["red", "yellow"]);
});

test("Filter test for Brand", () => {
    const type = 'car';
    expect(MeisterActions.filter(data, {type}, 'brand'))
        .toEqual([
            "Bugatti Veyron",
            "Ferrari F40",
            "Lamborghini Huracán",
